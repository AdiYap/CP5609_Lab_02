...
void touch_all (p_ListBoardObjects objectList)
{
	p_BoardObject toIterate;

	toIterate=objectList->theFirstObject();
	...
	while (NULL!=toIterate)
	{
		...
		toIterate=objectList->theNextObject();

	}

}

char game_all (p_ListBoardObjects objectList)
{
			  ...
			  { 
			      ...
			      { mySpace[player_id_x][player_id_y]->removeOnePlayer();
			        ...
			        startPlayer = new Player(player_id_x,player_id_y);
                                mySpace[player_id_x][player_id_y]->addObject(startPlayer);
